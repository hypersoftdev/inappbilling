...
- One-time products now support multiple purchase options/discount offers too, not just subscriptions. `ProductMapper` was reading the deprecated singular `ProductDetails.oneTimePurchaseOfferDetails` field (one price, no offer token) instead of `oneTimePurchaseOfferDetailsList`; `purchaseInApp()` never set an offer token at all. Both are fixed — `purchaseInApp()` now takes the same optional `offerId` parameter as `purchaseSubs()`/`updateSubs()`, and discounted one-time offers map to `RecurringMode.DISCOUNTED` (via `fullPriceMicros` being non-null) the same way discounted subscription phases already did.
